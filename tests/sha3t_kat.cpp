...
//
// Known-answer test for the sha3t Algorithm, the counterpart to sha256d_kat.cpp
// and blake2s_kat.cpp. Same question: not whether SHA3 is right, but whether the
// contract between struct work and the hash holds.
//
// The anchor is the strongest of the three: BitcoinIII makes sha3t its block
// hash as well as its proof of work, so a block's published identity *is* the
// sha3t digest of its header. nBits is checked too, and not for redundancy --
// the digest anchors the hash, the difficulty anchors the vector, and a
// mistyped byte in either would fail only the first.
//
...
// constant match. Everything is compared as bytes, not words, because a word
// comparison bakes this host's endianness in.
...
// fulltest() compares. Bitcoin's compact encoding, which BitcoinIII inherited:
...
// One vector: hash() must reproduce the digest the network published as this
// block's identity, that digest must clear the difficulty the header itself
// states, and verify() must agree with both.
...
int main()
{
    ...
    std::unique_ptr<vkminer::Algorithm> algo = vkminer::create_algorithm("sha3t");
    if (!algo) {
        std::printf("FAIL: the registry has no 'sha3t'\n");
        return 1;
    }
    ...
    if (std::strcmp(algo->name(), "sha3t") != 0)
        fail("create_algorithm(\"sha3t\") returned something else");
    if (!vkminer::create_algorithm("SHA3T"))
        fail("--algo is case sensitive, and should not be");

    // sha3d and keccak are real algorithms and neither is this one -- two
    // hashes rather than three, and Keccak's padding rather than SHA3's.
    // Answering to either name means mining it with the wrong hash.
    if (vkminer::create_algorithm("sha3d"))
        fail("the registry answered to sha3d, which is a different algorithm");
    if (vkminer::create_algorithm("keccak"))
        fail("the registry answered to keccak, which is a different padding");

    // BC3 is Bitcoin with the block hash swapped, so a difficulty of 1 means
    // 2^32 hashes here as it does there. Anything else and every difficulty
    // this miner prints is off by that factor, silently.
    if (algo->target_factor() != 1.)
        fail("sha3t quotes difficulty on the Bitcoin scale, and target_factor "
             "says %g", algo->target_factor());

    const vkminer::KnownAnswer *answers = nullptr;
    size_t count = algo->known_answers(&answers);
    if (count == 0 || !answers) {
        std::printf("FAIL: sha3t publishes no known answers\n");
        return 1;
    }
    ...
    std::printf("sha3t: %zu mined block headers hash to their published block "
                "hash, clear their own nBits, and verify correctly\n", count);
    return 0;
}
