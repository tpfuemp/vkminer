...
| `firopow` | Working. Pool shares accepted. The same code over Firo's constants: 1300-block epochs, a new program every block, and a dataset sized 64 epochs ahead of the one it seeds from -- **10.7 GiB in August 2026**, so a card under 12 GiB cannot mine it at all |
| `evrprogpow` | Working. Pool shares accepted. KawPoW's round over Evrmore's epoch: 12000 blocks, and a dataset that starts at 3 GiB rather than 1 GiB -- **4.3 GiB in August 2026**. Nobody published a test vector for it, so the self-test checks every kernel against this build's own reference and says so |
| `meowpow` | Working. Pool shares accepted. The one fork that changes the round itself -- 16 registers, 6 cache reads and 9 arithmetic operations, over a program that lasts six blocks -- and the one that *scales* its dataset instead of offsetting it: four times the epoch from epoch 110 on, **9.5 GiB in August 2026**. No published vector for this one either, so the self-test says the same thing it says for `evrprogpow` |
| `meraki` | Working. Pool shares accepted. Telestai's round: twelve cache reads against five arithmetic operations, over half as many rounds as KawPoW, on epochs of 27500 blocks -- **1.3 GiB in August 2026**, the smallest dataset in the family. Its keccak seal is Ravencoin's, so the round and the epoch are all that separate the two chains. No published vector for this one either, and the self-test says so |

Those five dataset sizes carry a date because they only grow. Every ProgPoW chain adds about 8 MB
to its dataset each epoch, which at the block times of August 2026 is roughly 6 MB a day for
`meowpow`, 4 for `firopow` and 1 for `evrprogpow` -- so `firopow` needs a 12 GiB card now and
`meowpow` will before long, while `evrprogpow` and `meraki` have years of room. Check the current
height against your card before planning around a number here.
